...
export async function checkXAccountStatusByUsername(username) {
  const screenName = String(username || '')
    .trim()
    .replace(/^@+/, '');
  if (!screenName) return { status: 'unknown' };
  ...
  if (userRes.httpStatus === 0 && !userRes.data) return { status: 'unknown' };
  if (userCode === 403 || userMsg.includes('SUSPEND')) return { status: 'suspended' };
  if (userCode === 401 || userMsg.includes('PRIVATE') || userMsg.includes('PROTECTED')) return { status: 'ok' };
  if (userCode === 404 || userMsg.includes('NOT FOUND')) return { status: 'deleted' };
  if (hasUser(userRes.data) && (userRes.ok || userCode === 200)) return { status: 'ok' };
