...
export async function probeMediaBytes(mediaUrl, { signal } = {}) {
  ...
  try {
    const res = await fetch(`/api/download/size?url=${encodeURIComponent(mediaUrl)}`, {
      credentials: 'include',
      cache: 'no-store',
      signal,
    });
