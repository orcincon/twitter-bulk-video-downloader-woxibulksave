...
function isMissingRpc(status, body) {
  return (
    ...
    body.includes('42883') ||
    /record_site_visit/i.test(body) ||
    /could not find the function/i.test(body)
  );
}

function classifyWriteError(status, body) {
  if (status === 404 || body.includes('42P01') || body.includes('site_visits')) {
    return 'TABLE_MISSING';
  }
  if (status === 401 || status === 403 || body.includes('42501')) {
    return 'RLS_DENIED';
  }
  return body.slice(0, 200) || `HTTP_${status}`;
}

/** Sayaç RPC (019); yoksa eski site_visits satırına düşer. Edge middleware uyumlu. */
export async function recordSiteVisit(row) {
  ...
  try {
    const rpcRes = await fetch(`${baseUrl}/rest/v1/rpc/record_site_visit`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        p_visitor_key: row.visitor_key,
        p_path: row.path || '/',
    ...
    if (!isMissingRpc(rpcRes.status, rpcBody)) {
      if (rpcRes.status === 401 || rpcRes.status === 403 || rpcBody.includes('42501')) {
        return { ok: false, error: 'RLS_DENIED' };
      }
      return { ok: false, error: rpcBody.slice(0, 200) || `HTTP_${rpcRes.status}` };
    }
    ...
    return { ok: false, error: classifyWriteError(res.status, body) };
  } catch (err) {
