 ...
 * Oturumdaki access_token yalnızca DB'de token yoksa yazılır; süresi dolmuş kayıtlar refresh ile yenilenir.
 */
export async function ensureUserInSupabase(session) {
  ...
  if (sessionToken) {
    let rowRes = await supabase
      .from('users')
      .select('access_token, token_is_valid, refresh_token')
      .eq('id', user.id)
      .maybeSingle();
    if (rowRes.error) {
    ...
    if (rowRes.error) {
      console.warn('[supabase] ensureUser token check:', rowRes.error.message);
    } else {
      const hasStored = Boolean(rowRes.data?.access_token?.trim());
      const hasRefresh = Boolean(rowRes.data?.refresh_token?.trim());
      if (!hasStored && !hasRefresh) {
