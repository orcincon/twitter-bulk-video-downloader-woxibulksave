...
import { invalidateOAuthTokenPoolCache } from './oauth-token-pool.js';
  ...
  callbacks: {
    async jwt({ token, account, user }) {
      if (account?.access_token) {
        ...
        token.expires_at = account.expires_at || Math.floor(Date.now() / 1000) + 7200;
      }
      ...
      const stillFresh = expiresAtMs > 0 && Date.now() < expiresAtMs - 60_000;
      if (!account && !stillFresh && token.sub) {
        try {
          const supabase = createSupabaseClient();
          if (supabase) {
            let rowRes = await supabase
              .from('users')
              .select('id, access_token, refresh_token, token_expires_at, token_is_valid')
              .eq('id', token.sub)
              .maybeSingle();
            if (rowRes.error) {
            ...
            const fresh = await ensureFreshUserAccessToken(supabase, rowRes.data);
            if (fresh) {
              token.access_token = fresh;
              token.expires_at = Math.floor(Date.now() / 1000) + 7200;
          ...
          console.warn('[auth] jwt refresh:', err?.message || err);
        }
      }
      return token;
    },
